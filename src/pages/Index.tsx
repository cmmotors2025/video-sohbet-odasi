  ...
  return (
    <div className="min-h-screen cinema-gradient flex flex-col items-center justify-center p-6">
      ...
      <button
        ...
        className="absolute top-4 right-4 w-10 h-10 rounded-full bg-secondary/50 border border-border/50 flex items-center justify-center overflow-hidden hover:border-primary/50 transition-colors"
      >
        {profile?.avatar_url ? (
      ...
      <div className="w-full max-w-sm space-y-4 animate-slide-up">
        <Button
          ...
          disabled={isCreating}
          className="w-full h-14 text-lg font-medium bg-primary hover:bg-primary/90 glow-primary"
        >
          {isCreating ? (
          ...
          <div className="space-y-3 animate-fade-in">
            <div className="relative">
              <Input
                value={roomCode}
                onChange={(e) =>
                  setRoomCode(e.target.value.replace(/\D/g, '').slice(0, 5))
                }
                placeholder="5 haneli oda kodu"
                className="h-14 text-lg text-center tracking-[0.5em] bg-input border-border/50 placeholder:tracking-normal"
                maxLength={5}
                inputMode="numeric"
                autoFocus
              />
